...
% build tuples for words with occur information
entry([], _S)->[];
entry([X|Xs], S)->[{X, lists:reverse(sort_occurs(lists:reverse(occurs(X, S))))}|entry(Xs, S)].

% convert occur list to list of tuples
...
% check word occur in lists created form source text
...
% convert lines of words to lists of separated words
lines_to_words_list([])->[];
...
% remove words with length <= 3 symbols
remove_short([])->[];
remove_short([X|Xs])->
  case X of
...
% remove all non literals symbols from string
clear_str([])->[];
